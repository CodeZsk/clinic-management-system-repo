...
def hideAllFrames(add_patient_frame, manage_patient_frame, add_appointment_frame, manage_appointmnet_frame, show_patient_info_frame, show_patient_Appointment_info_frame):
...
def addAppointment(add_patient_frame, manage_patient_frame, add_appointment_frame, manage_appointmnet_frame, show_patient_info_frame, show_patient_Appointment_info_frame, conn, search_id):
    # Iqra
    patient_id = search_id.get()
    if patient_id.isnumeric():
        hideAllFrames(add_patient_frame, manage_patient_frame, add_appointment_frame,
                      manage_appointmnet_frame, show_patient_info_frame, show_patient_Appointment_info_frame)
        add_appointment_frame.pack(fill='both', expand=1)
        addAppointmentBody(add_appointment_frame, conn, patient_id)
    else:
        print("Hello World!")
        pass


def addAppointmentBody(frame, conn, patient_id):
    cur = conn.cursor()
    cur.execute(
        "select p_info.patient_id, p_info.first_name, p_info.middle_name, p_info.last_name, p_info.age, p_info.gender, p_info.blood_group, p_info.phone_no, p_info.email_id, p_info.doctor_name,p_info.digonosis, p_info.date, p_info.time  from patient_info p_info where p_info.patient_id=%s " % patient_id)
